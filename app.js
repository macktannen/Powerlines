...
window.APP_VERSION = '3.16.53';
window.APP_BUILD_TIME = '2026-08-10 12:05:00 EST';

console.log(
  '%c ⚡ Indiana Power Grid Viewer %c v3.16.53 ',
...
function recalculateFlightPlan() {
  ...
  if (state.flightPlanner.autoPlanBackground !== false && state.flightPlanner.circuitLegs.length > 1) {
    ...
    if (state.flightPlanner.lockFirstCircuit && state.flightPlanner.circuitLegs.length > 1) {
      ...
      if (firstObj) {
        const ep = findCircuitEndpoints(firstObj, optStart);
...
function generateFlightPlannerPdfReport() {
  ...
  <body>
    ...
    <script>
      ...
      map.on('zoomend moveend', updateDynamicAnchors);
    </script>
  </body>
  </html>`;
